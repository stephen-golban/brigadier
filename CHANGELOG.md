...
- `brigadier init` no longer exits without asking anything when the installer
  runs it. Under Bun, `process.stdin` on a freshly opened `/dev/tty` never
  yields a line and never ends, and `install.sh` hands init exactly that
  descriptor — so a `curl | sh` install printed the first question, exited 0,
  and left no config behind while looking like it had succeeded. stdin is now
  read from descriptor 0 directly.
...
[unreleased]: https://github.com/stephen-golban/brigadier/compare/v0.2.1...HEAD
[0.2.1]: https://github.com/stephen-golban/brigadier/compare/v0.2.0...v0.2.1
